...
- Click **📥 Items CSV** in the header — this button now works in **both** static and Supabase-mode warehouses (editor/maintainer/admin; hidden for viewers in Supabase mode). In Supabase mode, resolved/new rows are written straight to the database (`items.insert`/`.update`, resolving `Manufacturer` the same way the Add/Edit modal does) instead of the local encrypted draft — everything else about the conflict flow below is identical either way.
- Select your items CSV file
- A BTK number only in the file is added as a new item, no review needed. A BTK number that already exists in this warehouse is a **conflict** — you'll be walked through each one (one at a time, using the same merge-view grid as manually merging items — see [Duplicate, Remove & Merge](#features)), picking per field whether the database's current value or the CSV's incoming value wins; nothing is written until every conflict is resolved, and cancelling drops the entire import with no changes made. A column your CSV doesn't have at all is never a conflict candidate — the database's existing value for it is kept automatically, so a shorter/older export can't accidentally blank out newer fields it was never aware of. A BTK number that exists in the database but isn't mentioned in the CSV at all is left completely untouched — import no longer wholesale-replaces the list.
...
- **Default view**: Supabase-mode warehouses open to **🏭 Manufacturers** rather than Items (static warehouses still open to Items — there's no dedicated Manufacturers entity there, just a free-text field, so the tab itself is hidden).
