...
**Changed format, if you set any codes before this**: `LocationCode` used to be 5 parts (`Zone-Aisle-Rack-Level-Bin`, e.g. `A-03-2-4-07`); it's now 4 (`ZoneDepth-Level-Bin`, e.g. `A2-4-07`) — Aisle is gone, and the old "Rack" number is now "Depth", directly appended to the zone letters. `supabase/schema_location_code_v2.sql` migrates any already-saved values automatically (old Aisle is simply dropped, Rack becomes Depth, Level and Bin carry over unchanged) before tightening the `CHECK` constraint to the new shape.

**Setting a code** (quick-edit or the Add/Edit modal): pick Zone/Depth/Level from dropdowns — built from the warehouse's actual rack areas, via the same `getAvailableZones()`/`getZoneBounds()` the Bin Location Map uses — and type just the Bin number, or ignore the dropdowns entirely and type the whole code by hand in the text field underneath. The two stay loosely in sync either way: a complete picker selection writes the composed code into the text field, and typing a recognizable code into the text field updates the dropdowns to match (a half-typed code is left alone rather than fighting you). The ℹ️ button next to the field label explains the format with one worked example. If the warehouse has no rack areas defined yet, the dropdowns are empty and a hint points you at the Warehouse page's Layout Designer — typing by hand still works regardless.
...
- **Inventering** (stocktaking, 📋 icon in the header) — a dedicated counting-session mode, separate from Pack Order. Setup: pick a Rack Area (or "All areas") or a Kit to seed a checklist, then fine-tune with checkboxes ("Select All"/"Select None" or uncheck individual items) before hitting "▶️ Start Inventering". The session itself is a plain done/not-done list, sorted by bin location (same order as **Sort: Bin Location** — walk the floor once, front-to-back) — each row shows the system's recorded stock number, the item name, its BTK/location, and a "✅ Done"/"⬜ Not Done" toggle you can flip either way. Tapping a row (not the toggle) opens that item's full detail page; "← Back" returns to the session. It's session-local like Pack Order — not saved anywhere, doesn't require write permission, and re-clicking the 📋 icon resumes an in-progress session instead of restarting the setup wizard; "🗑️ Clear Session" ends it and goes back to setup. Marking an item **Done** (not un-marking it) does write one thing: `items.last_inventoried_at`/`_by`/`_location` (a snapshot of `location_code` at the time, not a live reference), shown on the item's own page as "📋 Last inventoried: …" and as a small date under its row in the next session. Only the *last* count is kept, not a full history log.
...
- **`items`** — `btk` (text, PK), `warehouse_id` (FK), `manufacturer` (text, denormalized display copy kept in sync with `manufacturer_id`), `manufacturer_id` (FK → `manufacturers.id`, nullable), `itemnumber`, `itemname_en`, `itemname_sv`, `itemnumber2`, `itemnumber3`, `numberofitems` (int), `inventorylocation`, `map_position` (text, `A1`–`F6`), `location_code` (text, `ZoneDepth-Level-Bin` e.g. `A1-4-07`, CHECK-constrained format — see [Bin Location Codes](#bin-location-codes)), `comments`, `images` (legacy, unused for Supabase-mode items), `image_full_url`, `image_thumb_url`, `updated_at`/`updated_by` (set by a trigger on every write), `last_inventoried_at`/`_by`/`_location` (set client-side when a stocktake marks the item Done — a snapshot of `location_code` at that moment, not a live reference — see [Inventering](#features) — `_location`/`_at`/`_by` all stay `null` until the item's first count).
