...
- **Manage Warehouses** (admin only, Settings → "🏬 Manage Warehouses"): Warehouses 2/3 are separate offline encrypted vaults, not connected to Supabase — that's how the project was built, not a UI setting. The switcher hides them by default (a per-device `localStorage` flag, toggleable in this modal — never hides whichever warehouse is currently open, so you can't strand yourself) so it only lists warehouses actually on Supabase. "+ Add Warehouse" creates a new Supabase-backed warehouse by name/address with an auto-suggested free ID (`schema_warehouse_insert.sql` adds the admin-only `insert` policy this needs — `warehouses` never had one before). Nothing else needs new RLS for a freshly created warehouse to work: every `items`/`kits`/`warehouse_zones`/etc. policy already compares against whatever `warehouse_id` a row actually has, not a hardcoded `'1'`.
