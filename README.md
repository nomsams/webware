...
**Planned next**: overlaying a grid directly on an uploaded rack photo, mapping image regions to Level/Bin — `warehouse_rack_images.grid_overlay` (jsonb) is reserved for that, but the overlay mechanism itself hasn't been designed yet; today a photo is just attached to a (zone, aisle, rack), no overlay.
...
- **🖊️ Draw** (default): drag across the grid to draw a new zone. It's labeled automatically in sequence — `A`, then `B`, `C`, and so on — sized to however many cells you dragged across (that becomes its Aisle × Rack count directly: a zone drawn 5 cells wide is 5 aisles). Click an existing zone to open an inspector for its label and Level/Bin counts (not drawable — they're shelf depth, not floor space) or to delete it.
...
**Aisle/Rack Photos**: pick a zone (from either a drawn/manual zone or one only seen in an item's `location_code`), enter an aisle and rack number, and upload — editor or admin, same role as item photo uploads. Each (zone, aisle, rack) holds one photo; re-uploading replaces it. This is what `warehouse_rack_images` is for (see the table reference above) — grid-overlay mapping onto the photo itself is a planned next step, not built yet.
...
- **`warehouse_zones`** — optional, usually empty until you use the Layout Designer (see [Warehouse Page & Layout Designer](#warehouse-page--layout-designer)). `id` (bigint identity, PK), `warehouse_id` (FK), `zone` (text, e.g. `"A"` — matches a `location_code`'s Zone component), `label` (optional human-friendly name), `max_aisle`/`max_rack`/`max_level`/`max_bin` (smallint — `max_aisle`/`max_rack` double as the drawn block's width/height in grid cells), `grid_col`/`grid_row` (smallint, the block's position on the designer's drawing grid). Explicitly bounds a zone for both the Layout Designer and the item-detail Bin Location Map grid (see [Bin Location Codes](#bin-location-codes)) when you want it bigger than what's currently stocked, or want a zone labeled/positioned; with no row for a zone, the Bin Location Map auto-detects bounds from the highest aisle/rack it finds in `items.location_code` (it just can't be drawn on the designer's grid without a row, since auto-detection has no position to place it at). Admin-write, readable to the zone's own warehouse (or admin).
- **`warehouse_rack_images`** — one row per photographed rack. `id` (bigint identity, PK), `warehouse_id` (FK), `zone`, `aisle`, `rack` (smallint, composite-unique with `warehouse_id`+`zone`), `image_url`, `grid_overlay` (jsonb, **still unused** — reserved for whatever calibration data a future image-to-grid overlay feature needs, so mapping Level/Bin positions onto the photo itself won't require another migration), `created_at`. Uploaded from the Warehouse page (editor/admin); each (zone, aisle, rack) has at most one photo — re-uploading replaces it (upsert). Backed by the `rack-images` Storage bucket (public-read, editor/admin write — same shape as `item-images`).
